...
# SHIC TODO Change these urls to shibainucoin.net and add a depends subdomain which will do what this does
# Dependencies
...
Standalone script to perform the gitian build of Shibacoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/shibacoinppc/shibacoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/shibacoin-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/shibacoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean shibacoin git directory because of old caching
    if [ -d inputs/shibacoin/ ]; then
        echo "Cleaning Shibacoin directory..."
        rm -rf inputs/shibacoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit shibacoin="$COMMIT" --url shibacoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
