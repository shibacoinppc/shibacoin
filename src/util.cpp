...
#else
    const char* pszModule = "shibacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shibacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Shibacoin";
#else
    // Unix
    return pathRet / ".shibacoin";
#endif
#endif
}
