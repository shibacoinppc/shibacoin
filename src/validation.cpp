...
const std::string strMessageMagic = "Shibacoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetShibacoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Shibacoin: Switch maturity at depth 145,000
...
bool DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    const CTxOut &prevout = view.AccessCoins(tx.vin[j].prevout.hash)->vout[tx.vin[j].prevout.n];
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
    ...
    if (fAddressIndex) {
        if (!pblocktree->EraseAddressIndex(addressIndex)) {
            return error("DisconnectBlock(): Failed to delete address index");
        }
        if (!pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("DisconnectBlock(): Failed to write address unspent index");
        }
    }
...
void ThreadScriptCheck() {
    RenameThread("shibacoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Shibacoin: BIP30 has been active since inception
    ...
    // Shibacoin: BIP16 has been enabled since inception
            ...
            if (fAddressIndex || fSpentIndex)
             {
                 for (size_t j = 0; j < tx.vin.size(); j++) {
                     ...
                     const CTxIn input = tx.vin[j];
                     const CTxOut &prevout = view.AccessCoins(tx.vin[j].prevout.hash)->vout[input.prevout.n];
                     uint160 hashBytes;
                     int addressType;
    ...
    CAmount blockReward = nFees + GetShibacoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Shibacoin: Disallow AuxPow blocks before it is activated.
    ...
    // Shibacoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Shibacoin: We probably want to disable this
    int nLockTimeFlags = 0;
