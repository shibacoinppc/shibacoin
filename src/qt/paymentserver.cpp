...
const QString BITCOIN_IPC_PREFIX("shibacoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (QFile::exists(arg)) // Filename
        {
            ...
            {
                if (request.getDetails().genesis() == "ff271edcc83f7d71e7a4e4b0a43b386a188e1470a28671cdbdc47e900118ac7f")
                {
                    SelectParams(CBaseChainParams::MAIN);
                }
                else if (request.getDetails().genesis() == "3930d62aeedce995d507b93eb6a937b142a45a7db30d389257a20b1bceaf105d")
                {
                    SelectParams(CBaseChainParams::TESTNET);
                }
            }
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(ipcServerName)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start shibacoin: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Shibacoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
