...
/* Dogecoin 1.14.5: set the wallet's discard threshold to 1 SHIC because that's what 97%
 ...
 */
/* Dogecoin 1.14.6: set the wallet's discard threshold to 0.01 SHIC. Very network
...
/*
 * Shibacoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Shibacoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Shibacoin: Get a fee targetting a specific transaction speed.
     */
    CAmount GetShibacoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Shibacoin: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetShibacoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Shibacoin dust fee.
     */
